...
def decode_bitchunks(chunk_bits, chunks):
    ...
    for n in out_bytes:#add code
        at.append(n)   #add code
    return out_bytes
            ...
            try:
                ...
                if "201502059" in byte_stream:#add code
                   ...
                   for n in range(int((len(b)/2))):
                        ...
                        sample=numpy.sin(2*pi*rdom[n*2]/2*time).astype(numpy.float32)
                        #print(sample)
                        stream.write(sample)
                        sample=numpy.sin(2*pi*rdom[n*2+1]/2*time).astype(numpy.float32)
                        #print(sample)
                        stream.write(sample)
